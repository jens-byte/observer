            ...
            VStack(spacing: 0) {
                ...
                if viewModel.isLoading && viewModel.sites.isEmpty {
                    ...
                    ContentUnavailableView(
                        "No Sites",
                        systemImage: "globe",
                        description: Text("Add your first site to start monitoring")
                    )
                    Spacer()
                } else {
                    List {
                            ...
                            Section(group) {
                                ForEach(sites) { site in
                                    ...
                                    .swipeActions(edge: .trailing, allowsFullSwipe: false) {
                                        if authManager.canEdit {
                                            Button(role: .destructive) {
                                                Task {
                                                    await viewModel.deleteSite(site)
                                                }
                                            } label: {
                                    ...
                                    .swipeActions(edge: .leading) {
                                        Button {
                                            Task {
                                                await viewModel.toggleStar(site)
                                            }
                                        } label: {
                                            Label(
                                                site.isStarred ? "Unstar" : "Star",
                                                systemImage: site.isStarred ? "star.slash" : "star"
                                            )
                                        }
                                        .tint(.yellow)
                                    }
